...
public class FieldFunctions {
    ...
    public static Function<String, List<String>> format(ColumnFormat cf) {
        return (s) -> Splitter.on(Pattern.compile("\\s+"))
                .trimResults().splitToList(s)
                .stream()
                .reduce(new LinkedList<>(), accumulator(cf.getWidth()), combiner())
                .stream()
                .map((str) -> cf.getAlign().get().apply(str.trim(),cf.getWidth()))
